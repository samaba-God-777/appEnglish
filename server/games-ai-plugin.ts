...
/** Per-game default round counts (kept ≥ each game's engine ROUNDS so UIs don't shrink). */
...
/** Validate an items array; fall back to [] when malformed so the client uses static content. */
function sanitize(game: string, items: unknown): unknown[] {
  if (!Array.isArray(items)) return [];
  switch (game) {
    case "quiz":
      return items.filter(isMultiChoice).slice(0, 8);
    case "sentence-builder":
    case "typing-race":
    case "dictation":
      return items.filter(isSentence).slice(0, 8);
    case "word-scramble":
    case "hangman":
    case "memory-match":
      return items.filter(isWordPair).slice(0, 8);
    case "idiom": {
      const seen = new Set<string>();
      const out: AnyItem[] = [];
      for (const i of items) {
        if (!isIdiom(i) || seen.has(i.meaning.toLowerCase())) continue;
        seen.add(i.meaning.toLowerCase());
        out.push(i);
    ...
    case "story": {
      const nodes = items.filter(isStoryItem).slice(0, 12);
...
/** Dev-server endpoint: POST /api/games/generate { game, level } → { items }. */
export function gamesAiPlugin(): Plugin {
  let apiKey: string | undefined;
  return {
    name: "englishai-games-ai",
    config(_config, { mode }) {
      apiKey = loadEnv(mode, process.cwd(), "").GROQ_API_KEY || process.env.GROQ_API_KEY;
    },
    configureServer(server) {
      server.middlewares.use("/api/games/generate", async (req, res) => {
        if (!apiKey) return sendJson(res, 503, { error: "GROQ_API_KEY not set" });
        if (req.method !== "POST") return sendJson(res, 405, { error: "Method not allowed" });
        try {
          const { game, level, count } = JSON.parse(await readBody(req)) as {
            game?: string;
            level?: string;
            count?: number;
          };
          ...
          if (items.length === 0) return sendJson(res, 502, { error: "No valid items generated" });
          sendJson(res, 200, { game, level: lvl, items });
        } catch (err) {
