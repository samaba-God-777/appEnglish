...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    // Simulated API latency; swap for a real auth service later.
      ...
      <div className="flex items-center justify-center p-6 lg:p-12">
        ...
        >
            ...
            {mode === "register" && (
              <div>
                ...
                <input
                  ...
                  type="text"
                  autoComplete="name"
                  placeholder="Degracia Williams"
                  className="h-11 w-full rounded-xl border border-border bg-card px-4 text-sm placeholder:text-muted-foreground focus:border-ring"
                  {...register("name")}
                />
              </div>
            )}
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            {mode === "login" ? "New to EnglishAI Pro?" : "Already have an account?"}{" "}
            <button
          ...
          <p className="mt-4 rounded-xl bg-muted px-4 py-3 text-center text-xs text-muted-foreground">
            Demo mode: any email + a password of 6+ characters works.
          </p>
        </motion.div>
      </div>
    </div>
