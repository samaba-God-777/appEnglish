...
import type { GrammarQuestion } from "../src/features/grammar/grammar-questions";
...
const isQuestion = (value: unknown): value is GrammarQuestion => {
...
/** Dev-server endpoints for Groq-powered grammar help. */
export function grammarAiPlugin(): Plugin {
  let apiKey: string | undefined;
  return {
    name: "englishai-grammar-ai",
    config(_config, { mode }) {
      apiKey =
        loadEnv(mode, process.cwd(), "").GROQ_API_KEY || process.env.GROQ_API_KEY;
    },
    configureServer(server) {
      server.middlewares.use("/api/grammar/chat", async (req, res) => {
        ...
        try {
          ...
          if (messages.length === 0) return sendJson(res, 400, { error: "Empty messages" });
          const reply = await complete(apiKey, TUTOR_SYSTEM(topic), messages, false);
          sendJson(res, 200, { reply });
        } catch (err) {
          const message = err instanceof Error ? err.message : "Grammar chat failed";
          console.error("[grammar-ai] chat:", message);
          sendJson(res, 502, { error: message });
        }
      });

      server.middlewares.use("/api/grammar/generate", async (req, res) => {
        if (!apiKey) return sendJson(res, 503, { error: "GROQ_API_KEY not set" });
        if (req.method !== "POST") return sendJson(res, 405, { error: "Method not allowed" });
        try {
          const { topicId, topicTitle, count } = JSON.parse(await readBody(req)) as {
            topicId?: string;
            topicTitle?: string;
            count?: number;
          };
          if (!topicId || !topicTitle) return sendJson(res, 400, { error: "topicId and topicTitle required" });
          const n = Math.min(Math.max(Number(count) || 5, 3), 8);
          const raw = await complete(
            apiKey,
            GENERATOR_PROMPT(topicTitle, topicId, n),
            [],
            true,
          );
          ...
          const questions = (parsed.questions ?? [])
            ...
            .map((q, i) => {
              ...
              return {
                ...
                id: `${topicId}-ai-${i + 1}`,
                topicId,
                kind: prompt.includes("___") ? "gapfill" : "mcq",
              };
            });
          if (questions.length === 0) return sendJson(res, 502, { error: "No valid questions generated" });
          sendJson(res, 200, { questions: questions as GrammarQuestion[] });
        } catch (err) {
          const message = err instanceof Error ? err.message : "Generation failed";
          console.error("[grammar-ai] generate:", message);
          sendJson(res, 502, { error: message });
        }
      });
    },
